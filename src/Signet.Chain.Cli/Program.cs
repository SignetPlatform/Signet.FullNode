      ...
      public static void Main(string[] args)
      {
         try
         {
            ...
            if (optionList.Contains("-help") || optionList.Contains("--help") || string.IsNullOrWhiteSpace(command))
            {
               ...
               //$!!$ tac builder.AppendLine(" dotnet run <Signet.Cli/Signet.Cli.dll> [network-name] [options] [method] <command> [arguments]");
               builder.AppendLine(" dotnet run <Signet.Cli/Signet.Cli.dll> [options] [method] <command> [arguments]");
               builder.AppendLine();
               ...
               builder.AppendLine();
               builder.AppendLine("dotnet run -testnet GET Wallet/history WalletName=testwallet - Lists all the historical transactions of the wallet called 'testwallet' on the stratis test network.");
               builder.AppendLine("dotnet run -rpcuser=stratistestuser -rpcpassword=signettestpassword -rpcconnect=127.0.0.3 -rpcport=26174 getinfo - Displays general information about the Signet node on the 127.0.0.3:26174, authenticating with the RPC specified user.");
               builder.AppendLine("dotnet run -rpcuser=btctestuser -rpcpassword=btctestpass getbalance - Displays the current balance of the opened wallet on the 127.0.0.1:8332 node, authenticating with the RPC specified user.");
            ...
            ////}
            //$tac
            networksSelector = Signet.Networks.SignetNetworks.Signet;
            //$
            ...
            if (!command.Contains("/"))
            {
               ...
               try
               {
                  ...
                  var nodeSettings = new NodeSettings(networksSelector: networksSelector, protocolVersion: ProtocolVersion.PROVEN_HEADER_VERSION,  agent: "SignetNode", args: options)
                  {
         ...
         catch (Exception err)
         {
            ...
            //$!!$ tacConsole.WriteLine(ExceptionToString(err));
            //$!!$ tac
            Console.Write(ExceptionToString(err));
            //$
            Console.ReadLine();
         }
      }
